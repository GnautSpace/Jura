...
const validateEnvironment = () => {
    ...
    if (!process.env.GEMINI_API_KEY_1) {
        errors.push("GEMINI_API_KEY_1 is missing");
    } else if (!process.env.GEMINI_API_KEY_1.startsWith('AIza')) {
        errors.push("GEMINI_API_KEY_1 appears to be invalid (should start with 'AIza')");
    }
    ...
    if (errors.length > 0) {
        ...
        console.error("  3. Add: GEMINI_API_KEY_1=your_api_key_here");
        process.exit(1);
    }
};
...
app.post("/chat", async (req, res) => {
    ...
    try {
        ...
        let model;
        try {
            model = genai_1.getGenerativeModel({
            ...
            throw new APIError(
                ...
                500,
                "Check your GEMINI_API_KEY_1 or try restarting the server"
            );
        }
        ...
        try {
            ...
            if (error.message.includes('API_KEY_INVALID')) {
                throw new APIError(
                    "Invalid API key",
                    401,
                    "Please check your GEMINI_API_KEY_1 in the .env file. Get a valid key from https://aistudio.google.com/"
                );
...
app.use((error, req, res, next) => {
    ...
    if (error instanceof ValidationError || error instanceof APIError || error instanceof ConfigurationError) {
        console.error(`HANDLED ERROR: ${error.name} - Status: ${error.statusCode}`);
        return res.status(error.statusCode).json({
            success: false,
            error: error.name,
            message: error.message,
...
const startServer = () => {
    const server = app.listen(port, () => {
        ...
        console.log(` Gemini AI: Ready (API Key: ${apiKey1.substring(0, 8)}...)`);
